...
import {
  ...
  FileUp, HelpCircle, AlertCircle, GripVertical, Edit2, Bell, LayoutDashboard, Check, BarChart3, FileText, Printer, MessageSquare, ExternalLink, MessageCircle, GitCommit, User, Flame, Trophy, FolderPlus, MessageSquareCode, MessageSquareText
} from "lucide-react";
...
try {
  ...
  console.warn("Firebase bulut bağlantısı kurulamadı:", e);
}
...
export default function App() {
    ...
    const unsubChats = onSnapshot(collection(db, 'artifacts', appId, 'public', 'data', 'chats'), (snapshot) => {
      if (!snapshot.empty) {
        ...
        setDoc(doc(db, 'artifacts', appId, 'public', 'data', 'chats', 'chat-genel'), {
          id: "chat-genel", type: "general", title: "Genel Ekip Sohbeti", messages: [{ id: "m-1", sender: "Sistem", text: "ASİSTAN bulut çalışma alanına hoş geldiniz.", time: "08:30" }]
        });
      }
    });
  ...
  return (
    <div style={styles.appShell}>
      ...
      <ChatBar chats={chats} setChats={setChats} currentUser={currentUser} db={db} appId={appId} isFirebaseActive={isFirebaseActive} firebaseUser={firebaseUser} isOpen={isChatBarOpen} setIsOpen={setIsChatBarOpen} />
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
        {todos.filter(t => t.user === currentUser.name).map(t => (
          <div key={t.id} style={{ background: "#0F172A", padding: 12, borderRadius: 8, display: "flex", justifyContent: "space-between" }}>
            <span>{t.text}</span>
            <button style={styles.deleteIconBtn} onClick={async () => { if(isFirebaseActive && firebaseUser) await deleteDoc(doc(db, 'artifacts', appId, 'public', 'data', 'todos', t.id)); }}><Trash2 size={14} /></button>
          </div>
        ))}
      </div>
    </div>
...
function ChatBar({ chats, setChats, currentUser, db, appId, isFirebaseActive, firebaseUser, isOpen, setIsOpen }) {
  const [msg, setMsg] = useState("");
  const activeChat = chats[0] || { id: "chat-genel", title: "Genel Ekip Sohbeti", messages: [] };
  ...
  const send = async (e) => {
    ...
    if (isFirebaseActive && firebaseUser) {
      await setDoc(doc(db, 'artifacts', appId, 'public', 'data', 'chats', activeChat.id || "chat-genel"), updated);
    } else {
    ...
    <>
      ...
      {!isOpen && (
        <button
          ...
          style={{
            position: "fixed", bottom: 20, right: 20, background: "#F59E0B", color: "#0F172A",
            border: "none", borderRadius: "50%", width: 56, height: 56, boxShadow: "0 4px 20px rgba(245, 158, 11, 0.4)",
      ...
      {isOpen && (
        <div style={{
          position: "fixed", bottom: 20, right: 20, width: 360, height: 480, background: "#1E293B",
          border: "1px solid #F59E0B", borderRadius: 16, boxShadow: "0 10px 30px rgba(0,0,0,0.5)",
          display: "flex", flexDirection: "column", zIndex: 1001, overflow: "hidden"
        }}>
          <div style={{ background: "#0F172A", padding: "12px 16px", borderBottom: "1px solid #334155", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
            <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
              <MessageCircle size={18} color="#F59E0B" />
              <span style={{ fontWeight: 800, fontSize: 13, color: "#F59E0B" }}>{activeChat.title || "Ekip Sohbeti"}</span>
            </div>
          ...
          <div style={{ flex: 1, overflowY: "auto", padding: 12, display: "flex", flexDirection: "column", gap: 10 }}>
            {activeChat.messages?.map(m => (
              <div key={m.id} style={{ alignSelf: m.sender === currentUser.name ? "flex-end" : "flex-start", maxWidth: "80%" }}>
                <div style={{ fontSize: 9, color: "#94A3B8", marginBottom: 2, textAlign: m.sender === currentUser.name ? "right" : "left" }}>
                  {m.sender} • {m.time}
                </div>
                <div style={{
                  background: m.sender === currentUser.name ? "#F59E0B" : "#0F172A",
                  color: m.sender === currentUser.name ? "#0F172A" : "#F8FAFC",
                  padding: "8px 12px", borderRadius: 10, fontSize: 12, fontWeight: m.sender === currentUser.name ? 700 : 400
                }}>
                  {m.text}
                </div>
              </div>
          ...
          <form onSubmit={send} style={{ padding: 12, background: "#0F172A", borderTop: "1px solid #334155", display: "flex", gap: 8 }}>
            <input
              style={{ ...styles.mainInput, fontSize: 11, padding: "8px 10px" }}
              placeholder="Mesaj yazın..."
              value={msg}
              onChange={e => setMsg(e.target.value)}
              autoFocus
            />
            <button type="submit" style={{ ...styles.primaryActionBtn, padding: "8px 12px" }}>
              <Send size={14} />
            </button>
          </form>
        </div>
  ...
  return (
    <div style={styles.loginOverlay}>
      <div style={styles.loginCard}>
        ...
        {error && <div style={styles.errorBar}>{error}</div>}
        <form onSubmit={e => { e.preventDefault(); onLogin(u.trim(), p.trim()); }} style={{ display: "flex", flexDirection: "column", gap: 14, marginTop: 10 }}>
          <div><label style={styles.inputLabel}>Kullanıcı Adı</label><input style={styles.mainInput} value={u} onChange={e => setU(e.target.value)} placeholder="Kullanıcı adınızı girin..." required autoFocus /></div>
          <div><label style={styles.inputLabel}>Şifre</label><input type="password" style={styles.mainInput} value={p} onChange={e => setP(e.target.value)} placeholder="Şifrenizi girin..." required /></div>
